...
def DSSData_from_PPNet(net,pm_bus,meas_bus, meas_line, meas_error, labels):
    ...
    - Input:
        - net: PandaPower Network, with detailed topology, buses, trafos and lines parameters, and load an generators.
        ...
        - meas_bus = {"vm_pu": meas_v, "va_rad": meas_phi, "p_mw": meas_p, "q_mvar": meas_q}: DataFrame of measurements v, phi, p and q on buses.
...
def preprocess_data(A_flat,B_flat,problem,grid):
    ...
    Inputs
        - A_flat: tf tensor of edge parameters and measurements of shape [n_samples, n_edges * n_edge_input], carrying topology parameters and input features
        - B_flat: tf tensor of bus parameters and measurements of shape [n_samples, n_buses * n_bus_input], carrying topology parameters and input features
        - problem: Instance of problem at hands, to get specific characteristics
        - grid: specifying grid for specific charactecteristics
...
class DeepStatisticalSolver2(keras.Model):
        ...
        for classe in range(self.hyper_edge_classes):
            ...
            for port in range(self.hyper_edge_port_number[classe]):
                ...
                self.phi_vertice[str(classe)+str(port)] = fc_improved(
                    ...
                    rate = self.rate,
                    l2_reg = self.l2_reg,
                    input_dim=1+(1+self.hyper_edge_port_number[classe])*(self.latent_dimension)+self.d_out[classe]+self.d_in[classe]
                )
    ...
    def call(self, A_flat, B_flat, A0):
        ...
        while self.time < 1.:
            ...
            self.time_edge_input = tf.ones([self.minibatch_size_tf, self.num_lines, 1]) * tf.constant(self.time, dtype=tf.float32)
            self.time_bus_input = tf.ones([self.minibatch_size_tf, self.num_buses, 1]) * tf.constant(self.time, dtype=tf.float32)
            ...
            self.Phi_edge_input = tf.concat([self.time_edge_input, self.H_v_from, self.H_v_to, self.H_e['0'], self.a_ij], axis=2)
            ...
            self.H_v += self.time_step_size * self.phi_vertice['10'](self.Phi_bus_input) + self.Phi_port1_sum + self.Phi_port2_sum
...
def get_pflow(y,A0, problem,grid):
   ...
   indices_from = tf.cast(A0[:,:,0], tf.int32)                  # tf.int32, [n_samples, n_edges, 1]
   indices_to = tf.cast(A0[:,:,1], tf.int32)                    # tf.int32, [n_samples, n_edges, 1]
   ...
   P_ij_from = - U1_i * U1_j * ( Y1_ij * tf.math.cos(U2_i - U2_j - A0[:,:,-1:]) + Y2_ij * tf.math.sin(U2_i - U2_j - A0[:,:,-1:])) \
                + (Y1_ij + Ys1_ij/2) * U1_i**2   # per unit

   Q_ij_from =  U1_i * U1_j * ( - Y1_ij * tf.math.sin(U2_i - U2_j - A0[:,:,-1:]) + Y2_ij * tf.math.cos(U2_i - U2_j - A0[:,:,-1:])) \
                - (Y2_ij + Ys2_ij/2) * U1_i**2    # per unit

    
   P_ij_to = - U1_i * U1_j * ( Y1_ij * tf.math.cos(U2_i - U2_j - A0[:,:,-1:]) - Y2_ij * tf.math.sin(U2_i - U2_j - A0[:,:,-1:])) \
            + (Y1_ij + Ys1_ij/2) * U1_j**2   # per unit
      
   Q_ij_to = U1_i * U1_j * ( Y1_ij * tf.math.sin(U2_i - U2_j - A0[:,:,-1:]) + Y2_ij * tf.math.cos(U2_i - U2_j - A0[:,:,-1:])) \
   ...
   if not grid=='stedin':
       I_ij_from = tf.concat([tf.cast(I_ij_from[:,:-n_trafo], dtype=tf.float32), tf.cast(I_ij_from[:,-n_trafo:] / 5.5, dtype=tf.float32)],axis=1)
   ...
   if not grid=='stedin':
        i_kat = tf.maximum(I_ij_from[:,-n_trafo:,:] * 110/25, I_ij_to[:,-n_trafo:,:] *20/25) *V_n *100
